...
export async function getDiagnosticsWorkspace(clinicId?: string | null): Promise<DiagnosticsWorkspace> {
  ...
  const [{ data: orders }, { data: patients }, { data: members }, { data: appointments }, { data: encounters }, { data: preference }] = await Promise.all([
  ...
  return {
    ...
    patients: patients ?? [],
    professionals: (members ?? []).map((member) => ({ id: member.id, full_name: (member.profile as unknown as { full_name?: string } | null)?.full_name ?? "Profissional" })),
