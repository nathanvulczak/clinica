...
function CommissionsPanel({ data, activeView }: { data: FinancialWorkspaceData; activeView: FinancialSubsection }) {
  ...
  const commissionTable = (
    <section className="rounded-lg border bg-card">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[920px] table-fixed text-[13px]"><thead className="bg-muted/40 text-left text-[11px] uppercase text-muted-foreground"><tr><th className="w-52 px-3 py-2.5">Profissional</th><th className="w-56 px-3 py-2.5">Origem</th><th className="w-28 px-3 py-2.5 text-right">Base</th><th className="w-28 px-3 py-2.5 text-right">Comissão</th><th className="w-32 px-3 py-2.5">Status</th><th className="w-52 px-3 py-2.5 text-right">Ações</th></tr></thead><tbody>{visibleCommissions.length ? visibleCommissions.map((item) => <tr key={item.id} className="border-t"><td className="truncate px-3 py-2.5 font-medium" title={item.professional?.profile?.full_name ?? "Profissional"}>{item.professional?.profile?.full_name ?? "Profissional"}</td><td className="truncate px-3 py-2.5" title={item.entry?.description ?? "Lançamento"}>{item.entry?.description ?? "Lançamento financeiro"}</td><td className="whitespace-nowrap px-3 py-2.5 text-right tabular-nums">{formatCurrencyBRL(item.base_amount_cents)}</td><td className="whitespace-nowrap px-3 py-2.5 text-right font-medium tabular-nums">{formatCurrencyBRL(item.commission_cents)}</td><td className="px-3 py-2.5"><Badge className={item.status === "paid" ? "bg-emerald-500/10 text-emerald-700" : item.status === "approved" ? "bg-sky-500/10 text-sky-700" : item.status === "cancelled" ? "bg-rose-500/10 text-rose-700" : "bg-amber-500/10 text-amber-700"}>{item.status === "paid" ? "Pago" : item.status === "approved" ? item.settlement_id ? "Programado" : "Aprovado" : item.status === "cancelled" ? "Cancelado" : "Pendente"}</Badge></td><td className="px-3 py-2.5"><div className="flex justify-end gap-1">{item.status === "pending" && data.access.canApprove ? <Button size="sm" variant="outline" onClick={() => setStatusTarget({ id: item.id, action: "approve" })}><CheckCircle2 />Aprovar</Button> : null}{item.status === "approved" && !item.settlement_id && data.access.canApprove ? <Button size="sm" onClick={() => setSettlementTargetId(item.id)}><CalendarCheck />Programar</Button> : null}{item.status !== "paid" && item.status !== "cancelled" && !item.settlement_id && data.access.canApprove ? <Button size="icon" variant="ghost" title="Cancelar" onClick={() => setStatusTarget({ id: item.id, action: "cancel" })}><XCircle /></Button> : null}{item.status === "paid" ? <span className="text-xs text-muted-foreground">{formatDate(item.paid_at)}</span> : null}</div></td></tr>) : <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">Nenhuma comissão nesta etapa.</td></tr>}</tbody></table></div>
    </section>
  );

  const settlementStatus: Record<string, string> = { draft: "Rascunho", approved: "Aprovado", scheduled: "Em aberto", paid: "Pago", reversed: "Estornado", cancelled: "Cancelado" };
  const settlementTable = (
    <section className="rounded-lg border bg-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><div><p className="font-medium">Acertos programados</p><p className="mt-1 text-sm text-muted-foreground">Cada acerto gera uma conta a pagar e mantém a origem das comissões rastreável.</p></div>{data.access.canApprove ? <Button size="sm" onClick={() => setSettlementTargetId(data.commissions.find((item) => item.status === "approved" && !item.settlement_id)?.id ?? "new")}><Plus />Programar acerto</Button> : null}</div>
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-[13px]"><thead className="bg-muted/40 text-left text-[11px] uppercase text-muted-foreground"><tr><th className="px-3 py-2.5">Profissional</th><th className="px-3 py-2.5">Período</th><th className="px-3 py-2.5 text-right">Itens</th><th className="px-3 py-2.5 text-right">Valor</th><th className="px-3 py-2.5">Vencimento</th><th className="px-3 py-2.5">Status</th><th className="px-3 py-2.5 text-right">Conta a pagar</th></tr></thead><tbody>{data.commissionSettlements.length ? data.commissionSettlements.map((item) => <tr key={item.id} className="border-t"><td className="px-3 py-2.5 font-medium">{item.professional?.profile?.full_name ?? "Profissional"}</td><td className="whitespace-nowrap px-3 py-2.5">{formatDate(item.period_start)} a {formatDate(item.period_end)}</td><td className="px-3 py-2.5 text-right tabular-nums">{item.commission_count}</td><td className="px-3 py-2.5 text-right font-medium tabular-nums">{formatCurrencyBRL(item.amount_cents)}</td><td className="whitespace-nowrap px-3 py-2.5">{formatDate(item.due_date)}</td><td className="px-3 py-2.5"><Badge className={item.status === "paid" ? "bg-emerald-500/10 text-emerald-700" : item.status === "reversed" || item.status === "cancelled" ? "bg-rose-500/10 text-rose-700" : "bg-amber-500/10 text-amber-700"}>{settlementStatus[item.status] ?? item.status}</Badge></td><td className="px-3 py-2.5 text-right">{item.payable_entry_id ? <Button asChild size="sm" variant="outline"><a href={`/financeiro?section=payables&view=${item.status === "paid" ? "reversals" : "open"}`}><Eye />Visualizar</a></Button> : "-"}</td></tr>) : <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Nenhum acerto programado.</td></tr>}</tbody></table></div>
    </section>
  );
  ...
  return (
    <div className="grid gap-5">
      ...
      {activeView === "settlements" ? settlementTable : null}

      {activeView === "receipts" ? commissionTable : null}
