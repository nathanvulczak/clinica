...
function DiagnosticsReportsPanel({ orders, criticalCount }: { orders: DiagnosticOrder[]; criticalCount: number }) {
  const categoryRows = Object.entries(categoryLabels).map(([key, label]) => {
    const rows = orders.filter((order) => order.category === key);
    return { key, label, total: rows.length, completed: rows.filter((order) => order.status === "completed").length, pendingItems: rows.reduce((sum, order) => sum + order.items.filter((item) => !item.results.some((result) => result.status === "final")).length, 0) };
  }).filter((row) => row.total > 0);
  const statusRows = Object.entries(statusLabels).map(([key, label]) => ({ key, label, total: orders.filter((order) => order.status === key).length })).filter((row) => row.total > 0);
  return <div className="grid gap-4"><div className="grid gap-3 lg:grid-cols-4"><div className="rounded-md border bg-card p-4"><FileSearch className="size-5 text-primary" /><p className="mt-3 text-sm font-semibold">Produção diagnóstica</p><p className="mt-1 text-xl font-semibold tabular-nums">{orders.length}</p><p className="text-xs text-muted-foreground">Pedidos no filtro atual.</p></div><div className="rounded-md border bg-card p-4"><CircleDot className="size-5 text-primary" /><p className="mt-3 text-sm font-semibold">Itens solicitados</p><p className="mt-1 text-xl font-semibold tabular-nums">{orders.reduce((sum, order) => sum + order.items.length, 0)}</p><p className="text-xs text-muted-foreground">Exames e procedimentos.</p></div><div className="rounded-md border bg-card p-4"><CheckCircle2 className="size-5 text-primary" /><p className="mt-3 text-sm font-semibold">Concluídos</p><p className="mt-1 text-xl font-semibold tabular-nums">{orders.filter((order) => order.status === "completed").length}</p><p className="text-xs text-muted-foreground">Pedidos finalizados.</p></div><div className="rounded-md border bg-card p-4"><AlertTriangle className="size-5 text-primary" /><p className="mt-3 text-sm font-semibold">Críticos</p><p className="mt-1 text-xl font-semibold tabular-nums">{criticalCount}</p><p className="text-xs text-muted-foreground">Resultados sinalizados.</p></div></div><section className="grid gap-4 lg:grid-cols-2"><div className="overflow-hidden rounded-md border bg-card"><div className="border-b px-4 py-3"><p className="text-sm font-semibold">Por categoria</p><p className="text-xs text-muted-foreground">Volume, conclusão e pendências por tipo de exame.</p></div><table className="w-full text-[13px]"><thead className="bg-muted/40 text-left text-[11px] uppercase text-muted-foreground"><tr><th className="px-3 py-2">Categoria</th><th className="px-3 py-2 text-right">Pedidos</th><th className="px-3 py-2 text-right">Concluídos</th><th className="px-3 py-2 text-right">Pendências</th></tr></thead><tbody>{categoryRows.length ? categoryRows.map((row) => <tr key={row.key} className="border-t"><td className="px-3 py-2.5 font-medium">{row.label}</td><td className="px-3 py-2.5 text-right tabular-nums">{row.total}</td><td className="px-3 py-2.5 text-right tabular-nums">{row.completed}</td><td className="px-3 py-2.5 text-right tabular-nums">{row.pendingItems}</td></tr>) : <tr><td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">Sem dados para o filtro.</td></tr>}</tbody></table></div><div className="overflow-hidden rounded-md border bg-card"><div className="border-b px-4 py-3"><p className="text-sm font-semibold">Por etapa</p><p className="text-xs text-muted-foreground">Acompanhamento operacional dos pedidos.</p></div><div className="grid gap-2 p-4">{statusRows.length ? statusRows.map((row) => <div key={row.key} className="grid grid-cols-[120px_1fr_42px] items-center gap-3 text-xs"><span className="font-medium">{row.label}</span><div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary" style={{ width: `${Math.max((row.total / Math.max(orders.length, 1)) * 100, 3)}%` }} /></div><span className="text-right font-semibold tabular-nums">{row.total}</span></div>) : <p className="py-8 text-center text-sm text-muted-foreground">Sem dados para o filtro.</p>}</div></div></section></div>;
}
